...
# safe_setReadable <- safely(~setReadable(.,orgDb, keyType = "ENTREZID"))
...
plot_objects <- list(GO_enrich_ul,KEGG,KEGG_mod,GSEA)
...
map2(plots_filtered,names(plots_filtered),function(x,y) {
  ggsave(x$result,width=11,height=12,units="in",file=file.path(out,paste0(y,".png")))
})
